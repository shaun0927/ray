...
def multiplexed(
    ...
    # (the bare `@serve.multiplexed` / `@serve.multiplexed()` form documented
    # on master) is treated as the legacy model-only path with size 3.
    ...
    if max_num_models_per_replica is not None:
        ...
        warnings.warn(
            ...
            "Example: @serve.multiplexed(name='model', size_per_replica=3)"
        )
        name = "model"
        size_per_replica = max_num_models_per_replica
        use_multiplex_model_legacy_metrics = True
    elif name is None:
        # Bare `@serve.multiplexed` (no kwargs): keep master's single-dim
        # semantics (name="model", size=3, legacy metric names). We don't
        # raise here — user-facing backward compatibility — but new code
        # should prefer `@serve.multiplexed(name="model", size_per_replica=N)`.
        name = "model"
        use_multiplex_model_legacy_metrics = True
    ...
    def _multiplex_decorator(func: Callable):
        ...
        # on this deployment (used to raise on typos).
...
def get_multiplexed_id(name: str) -> str:
    ...
    Args:
        name: The multiplex dimension name (e.g. "model", "session"). Must
            match a ``@serve.multiplexed(name=...)`` decorator on the
            deployment; otherwise a ``KeyError`` is raised to catch typos.
    ...
    """
    # Validate `name` against the dimensions registered at replica startup
    # via @serve.multiplexed(name=...) on the servable_object. Typos should
    # fail loudly rather than silently return "". `_multiplex_dimensions` is
    # empty when called outside a replica (e.g. in unit tests) — skip
